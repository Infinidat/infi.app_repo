...
class TasksTestCase(TestCase):
    ...
    def pull_msi(self, config):
        self.pull_file(config, "host-power-tools-0.18-windows-x86.msi", "/msi/x86")

    def pull_deb(self, config):
        self.pull_file(config, "host-power-tools-0.18-linux-ubuntu-natty-x86.deb",
                       "/deb/ubuntu/dists/natty/main/binary-i386")

    def pull_rpm(self, config):
        self.pull_file(config, "host-power-tools-0.18-linux-centos-6-x64.rpm",
                       "/rpm/centos/6/x86_64")
